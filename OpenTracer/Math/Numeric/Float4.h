...
namespace OpenTracerCore
{
	struct __declspec(align(16)) float4
	{
		...
		/* Minimum */
		friend inline const float4 f4min(const float4& v0, const float4& v1)
		{
			return _mm_min_ps(v0.xmm, v1.xmm);
		}

		/* Maximum */
		friend inline const float4 f4max(const float4& v0, const float4& v1)
		{
		...
		friend inline float4 mix(const float4& a, const float4& b, float l)
		{
#ifdef FLOAT4_USE_SSE
